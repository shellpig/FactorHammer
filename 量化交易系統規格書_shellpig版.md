...
| **10-H-2** | 實際移除與全專案回歸 | 10-H-1 驗收後 | 移除 `src/ui/`、舊 Streamlit 啟動腳本、`pyproject.toml` streamlit 三套件、舊 Streamlit 7 個 pytest 檔；更新四份文件；全專案 pytest 回歸 |
...
6. 當時的 Next.js dev launcher 在 desktop 與手機模擬器下皆可達 5 頁
...
2. 移除舊 Streamlit 啟動腳本（保留當時的 Next.js dev launcher / `run_api.bat` / `run_web.bat` / `run_dev.bat`）
...
| 11-E | UI/UX 收尾調整 | 工具名稱改 `FactorHammer`、版號 build-time 注入、警示文字字色統一黃、股東會無資料文案修正、報價列改一排、K 線右側加「前收」、股東會編輯按鈕內聯、散戶多空比 placeholder 移除 | 否 |
...
11-B 與 11-C 雖然資料層可分開設計，但 UI integration 共用 `dashboard-page-client.tsx`、`dashboard_service.py`、`api/routers/analysis.py`，因此不得並行實作，避免互相覆蓋。11-E 純前端調整，但仍排在 11-D 之後執行，避免與正在實作的 dashboard 元件互相覆蓋。
...
11-E 不新增資料、不新增 API、不動 Python；純前端版面與文案調整，搭配 build-time 注入版號。

##### 動作清單

| # | 項目 | 規格 |
|:---|:---|:---|
| 1 | 工具顯示名稱 | Sidebar header 顯示文字改為 `FactorHammer`；專案名稱、`pyproject.toml`、`web/package.json` `name`、git repo、Python package 名稱皆不動，僅 UI 字串改動 |
| 2 | 版號 | `FactorHammer` 右下方兩行 stack 顯示 `v{version}`，更小字體、灰色（與既有 muted token 對齊）；version 從 `web/package.json` 讀取，透過 build-time 注入 `NEXT_PUBLIC_APP_VERSION`（建議在 `next.config.ts` 讀 `package.json` 設環境變數），不打 API |
| 3 | 「資料源未提供」字色 | 既有「資料源未提供此標的的估值資料（如 ETF）」「資料源未提供此標的的月營收資料（如 ETF）」等說明文字，字色改用與「撈不到股東會資料，需要手動填入」一致的黃色 token；建議統一指向同一個 design token |
| 4 | 事件行事曆股東會文案 | `撈不到資料，需要手動填入（ETF沒有股東會）` → `撈不到股東會資料，需要手動填入（或是ETF沒有股東會）` |
| 5 | 報價列改一行 | 個股分析頁頂部報價區由原本多行 / 標題與數值分開排版，改為一排：`開盤  25.22　　最高  25.79　　最低  24.93　　前收  25.78　　成交量  36,464　　買量  438　　賣量  158`。每個欄位皆「中文標籤 + 全形空格 + 數值」串接；欄位之間用兩個全形空格分隔；標籤使用 muted 文字色、數值維持原有顏色規則（漲紅 / 跌綠 / 中性）。整列高度收為一排，下方所有區塊整體上移 |
| 6 | K 線右側「前收」標籤 | K 線圖右側價格軸區域，與既有「壓力 / 支撐」相同的「標籤 + 數字」格式新增「前收」標籤；標籤文字與數值整組同色，依台股慣例「漲紅跌綠」：今價 > 前收 → 紅、今價 < 前收 → 綠、今價 = 前收 → 灰；前收水平線本身仍維持目前畫法 |
| 7 | 股東會手動編輯按鈕位置 | 編輯按鈕從事件行事曆面板的右上角移到面板標題「事件行事曆」文字右側 8px 內聯、與標題同 baseline；按鈕外觀（icon / tooltip 文字 / onClick handler）不動 |
| 8 | 散戶多空比 placeholder 移除 | 左欄 chart 下方 6 個 dashed placeholder 中的「散戶多空比」整塊移除；該 placeholder 目前位於最底，移除後不影響其他區塊位置，不留空 row |

##### 11-E 不做

- 不改後端 API、不動 service、不動 Python 測試。
- 不動 `pyproject.toml`、不動 `web/package.json` 的 `name` 欄位、不更名 git repo、不動 Python package 名稱。
- 不做 splash screen、about dialog、version 詳細頁、changelog 頁。
- 不做主題、字體、字級的大規模調整；除項 5、項 6 明確指定者外，維持既有顏色與排版。
- 既有「壓力 / 支撐」標籤顏色規則不動。
- 不新增 `/api/version` 或任何後端 endpoint；版號是 build-time inject，不打 runtime API。

#### Phase 11 不做

- 不支援美股 P11 基本面 / 事件資料。
- 不做手機 <768px 的完整 P11 排版優化；11-A 可先隱藏或保守堆疊，後續依實測調整。
- 不做散戶多空比演算法；原 11-D placeholder 已改為 Goodinfo 股利政策 fallback；11-E 進一步將該 placeholder 整塊移除。
...
同步將舊 V2 啟動腳本改名為 `run_factorhammer.bat`，對齊 P11-E 已敲定的工具名稱 `FactorHammer`，並刪除舊腳本不保留 alias。
...
| 子階段 | 名稱 | 內容 | 執行角色 |
|:---|:---|:---|:---|
| 12-A | Portable Node.js + install.bat 改版 | install.bat 重整為 5 步驟、新增 portable Node 下載 + SHA-256 驗證、corepack/pnpm 啟用、舊 V2 啟動腳本改名為 `run_factorhammer.bat`、`web/package.json` 加 `packageManager` 欄、`.gitignore` 加 `tools/` | implementer |
...
| 修正 | 結尾訊息改為 `run_factorhammer.bat` |
...
| 變更 | 說明 |
|:---|:---|
| 改名 | 舊 V2 啟動腳本 → `run_factorhammer.bat`（內容照舊） |
| 刪舊 | 原舊 V2 啟動腳本直接刪除，**不保留 alias** |
...
| 改名 | 舊 V2 啟動腳本已刪除；`run_factorhammer.bat` 能正確啟動 dashboard；視窗標題顯示 FactorHammer |
...
舊啟動腳本名在所有現役文件（排除 `舊文件/` 與 V2.x 歷史變更紀錄段）**不得殘留**作為主啟動入口字眼。verifier 完成後跑：

```powershell
Get-ChildItem -Recurse -Include *.md -Exclude 舊文件 |
  Select-String "<舊 V2 啟動腳本名>|<舊 Streamlit 啟動腳本名>" |
...
### Phase 14：區網與遠端存取（多裝置使用）

#### Phase 14 定位

Phase 14 不新增資料源、不擴 AI、不動回測與 dashboard 邏輯，目標是讓既有 FactorHammer dev stack（FastAPI + Next.js）可被「**同一台 PC 之外**」的裝置使用，最常見情境是手機在家用瀏覽器連回主機跑 dashboard，或外出時透過 Tailscale 從任何網路連回家。

Phase 14 仍假設：
- PC 為 24×7 / 隨需開啟的 Windows 11 個人主機，FactorHammer 用 `run_factorhammer.bat` 啟動兩個 dev server。
- 不引入伺服器級 HTTPS、不引入認證、不引入雲端託管；資料層仍是本機 DuckDB + Parquet。
- 安全模型仍與「本機個人版」一致；遠端存取由 Tailscale / LAN 隔離擔保，不在 app 層加 auth。
...
#### 14-A：LAN / Tailscale 存取（proxy 同源）

##### 目標

1. 手機 / 平板 / 第二台筆電在**同一個 Wi-Fi** 下，瀏覽器打 `http://<PC LAN IP>:3000/dashboard` 即可看到 dashboard、所有資料正確載入、無 CORS 錯誤、無 fetch 失敗。
2. 安裝 Tailscale 後，**在外網**亦可用 `http://<PC tailscale IP>:3000/dashboard` 或 MagicDNS short name 連回家裡 PC。
3. PC 本機 `http://localhost:3000/dashboard` 與既有測試（vitest / pytest / Playwright）行為完全不變。
4. 即使日後 PC 的 LAN IP 改變、ISP 換、出差異地，**前端程式不需改設定**，使用者只需把瀏覽器 URL 換成新主機名 / IP。

##### 鎖定路徑：proxy 同源

14-A 鎖定「Next.js 反代 `/api/*` 到本機 FastAPI」的 proxy 同源方案。理由：

| 比較項 | proxy 同源（採用） | 環境變數 `NEXT_PUBLIC_API_URL`（否決） |
|:---|:---|:---|
| 前端 fetch 路徑 | 永遠相對路徑 `/api/...` | 絕對 URL，內含 PC IP |
| 換 IP 時 | 不用改任何檔 | 要改 `web/.env` 並 rebuild |
| CORS | 同源，不觸發 | 必須維護 `allow_origins` 白名單 |
| Tailscale / 公司 Wi-Fi / 出差網路 | 通用 | 每換一個來源就要更新一次 |
| 既有測試 | 不受影響 | apiBase mock 規則需重對 |
| 為來日 Inno Setup 打包準備 | 一致 | 需另做 build-time 注入 |

##### 不動的部分

1. **FastAPI CORS 白名單**：proxy 同源讓前端與後端在瀏覽器眼中皆為 `http://<host>:3000`，CORS 不會觸發。`api/main.py` 的 `allow_origins=["http://localhost:3000"]` 保持不動，避免破壞既有測試行為與安全姿態。
2. **api-client.ts 既有 `NEXT_PUBLIC_API_URL` 環境變數機制**：保留為 escape hatch（給 verifier / debug 用），預設讀不到時 baseURL 為空字串，走相對路徑。
3. **資料層 / 服務層 / 路由邏輯**：完全不動，後端仍只認 `http://127.0.0.1:8000` 為實際監聽位置。
4. **既有 vitest / pytest / Playwright 行為**：所有現有用 `localhost:3000` / `localhost:8000` 的測試與 mock 不需改寫。
...
實作細節請見 `開發設計方針.md` Phase 14 段落。

##### 安全模型

- `--host 0.0.0.0` 會讓 dev server 監聽**所有網卡**，亦即同 Wi-Fi 下的任何裝置（鄰居連你 Wi-Fi、IoT、客人手機）皆能打到 port 3000 / 8000。對個人版工具屬可接受範圍。
- 若採 Tailscale，雖然 PC 端仍綁 `0.0.0.0`，但 Tailscale interface 只對你帳號內的裝置開放，比 LAN 嚴格。
- 不在 app 層加 token / basic auth。若使用者要在公開 Wi-Fi 環境用，**應**先啟用 Tailscale，不在裸 LAN 上暴露。
- 不啟用 HTTPS。FastAPI / Next.js dev server 都以 HTTP 對外。需要 HTTPS 屬未來 phase。

##### 驗收條件（與測試指南對應）

- PC 端 `http://localhost:3000/dashboard` 仍可開、F12 Network 觀察到 `/api/...` 與 page 同 origin。
- PC 端瀏覽器直接打 `http://localhost:3000/api/data/symbols?market=tw` 回 200 JSON（驗證 rewrites 生效）。
- 同 Wi-Fi 另一裝置（手機 / 平板）打 `http://<PC LAN IP>:3000/dashboard`，能載入 2330 完整 dashboard、F12 Network 無 CORS 錯誤、無 ERR_CONNECTION_REFUSED。
- Tailscale 裝好後，手機關 Wi-Fi 開行動網路，打 `http://<PC tailscale 100.x IP>:3000/dashboard` 仍能連入。
- vitest / pytest / Playwright 既有測試全綠，無需改 mock。

##### Phase 14-A 不做

| 不做 | 理由 |
|:---|:---|
| HTTPS / 自簽憑證 | 需引入反向代理或新工具鏈，超出「個人版本機工具」範圍 |
| 使用者 / 密碼登入 | 安全擔保交給 Tailscale 帳號 / 家用 LAN 隔離 |
| 雲端託管後端 | 違背「零伺服器、本機運行」核心原則 |
| Inno Setup 打包整合 | 已列在未來打包章節，14-A 只負責 dev stack runtime 暴露面 |
| 對 WAN（公網 IP / DDNS / 路由器 port forward） | 安全與配置成本高；Tailscale 已涵蓋 |

##### Phase 14-A 風險

| 風險 | 影響 | 緩解 |
|:---|:---|:---|
| Windows Defender 第一次跳防火牆 prompt | 使用者不知道要按允許 | 安裝指南 / 啟動腳本說明在第一次跨裝置連線時會跳 |
| 同 Wi-Fi 上其他人能打到 dashboard | 資料外洩風險 | 文件明示在公開 Wi-Fi 應使用 Tailscale |
| LAN IP 變動 | 手機書籤失效 | 路由器設定 DHCP 保留；或推薦使用 Tailscale MagicDNS |
...
| 子階段 | 範圍 | 相依 |
|:---|:---|:---|
| 15-A-1 | DeepSeek Provider 接入 + 設定頁下拉 | 無 |
| 15-A-2 | `secrets/validate` 重構為 per-provider results | 15-A-1 完成（會新增 deepseek 欄位） |
...
| Secrets schema | `.env` 增加 `DEEPSEEK_API_KEY`；DeepSeek key 寫入路徑沿用既有 `PUT /api/config/secrets`（接受任意 provider dict），**不**在 15-A-1 改 `SecretsValidateRequest`（屬 15-A-2 範圍） |
...
| 5 | `web/src/hooks/use-config.ts` | **新增 `useConfig()` / `updateConfig()`**（既有 hook file 只有 `useSecretsStatus` / `useStrategyPresets` 等，無 config hook）。`useConfig()` 用 SWR GET `/api/config`、key `CONFIG_SWR_KEY = "/api/config"`；`updateConfig(patch)` PUT `/api/config` 後呼叫 `mutate(CONFIG_SWR_KEY)`。回傳型別用既有 `AppConfig` |
| 6 | `web/src/types/config.ts` | `SecretsStatus` interface 加 `deepseek: boolean`（與 `SecretsSection.PROVIDERS` 對齊；缺此欄位 tsc strict 會擋） |
| 7 | `web/src/components/settings/ai-toggle-section.tsx` | **重寫**：移除 disabled switch 與「永久停用」tooltip；新增 enable toggle、Provider 下拉、Model input；透過新增的 `useConfig` / `updateConfig` hook 寫 `PUT /api/config` patch（`{ai: {enabled, provider, model}}`）；**必須以 `useEffect` 在 SWR config 載入後同步 local state，並用 dirty flag 避免使用者編輯中被覆寫** |
| 8 | `web/src/components/settings/secrets-section.tsx` | `PROVIDERS` 陣列加一行 `{ key: "deepseek", label: "DeepSeek API Key" }`（位置建議放在 openai 後、anthropic 前，或統一排序） |
| 9 | `pyproject.toml` / `web/package.json` / `api/main.py` | version → `0.6.0`（**三處同步**） |
...
| `SecretsValidateRequest` 加 deepseek 欄位 | 屬 15-A-2；15-A-1 沿用 `PUT /api/config/secrets` 寫入 |
...
| DeepSeek JSON mode「機率回空 content」 | 偶發 `AICallError` | dashboard `_parse_dashboard_json` 已有兜底，回 None 後丟錯；不在 15-A-1 加 retry |
| `AiToggleSection` 重寫破壞既有 vitest「AI toggle disabled / tooltip」測試 | 既有 P10-G-2 測試壞 | 一併在本 sub 改寫對應測試（驗 enable / provider / model 可操作） |
| 使用者打開 AI toggle 但 `/api/ai/chat` 仍 503（15-B 未做） | Dashboard AI 分析可運作，但 chat 仍不能用 | 規格層接受：15-A-1 範圍只到 dashboard 路徑；chat 端點 503 + sidebar「後續開放」徽章保留到 15-C 才移除 |

### 15-A-2：secrets/validate 重構為 per-provider results + 移除 Google API Key alias

#### 目標

兩件事併同處理：

1. 把 `/api/config/secrets/validate` 從 FinMind-only 的 all-or-nothing contract，重構為 **FinMind 必填 + 其他 provider 選填 validate-if-present** 的 results map，支援部分成功與 per-provider 結果回傳。沿用 P12 onboarding 的 FinMind 強制安全模型，同時讓設定頁可同時驗多個 AI provider key 並逐欄顯示成功/失敗訊息。
2. **移除既有 `GOOGLE_API_KEY` / `google_api_key` / Google API Key UI 欄位的 alias 漂移**。現況 [src/ai/advisor.py:1074-1078](src/ai/advisor.py:1074) Gemini provider 同時讀 `gemini_api_key` 與 fallback `google_api_key`，[secrets-section.tsx:7-12](web/src/components/settings/secrets-section.tsx:7) UI 同時顯示「Gemini API Key」與「Google API Key」兩欄。一個 provider 兩個 UI 欄位是設計漂移；15-A-2 整理 secrets schema 時順手收掉。**遷移策略**：既有 `.env` 內 `GOOGLE_API_KEY` 保留（不主動刪），但 15-A-2 後系統不讀、不寫、不顯示；使用者要用 Gemini 請在設定頁填 Gemini API Key 寫入 `GEMINI_API_KEY`。

P15-A-2 完成後 supersede P12-C / 10-G-2 的 google status/UI 描述；Google API Key 欄位為 legacy，後續不讀不寫不顯示。
...
| 7 | `src/core/config.py` | `get_config()` 內 `config["secrets"]` 組裝段**移除** `"google_api_key": os.getenv("GOOGLE_API_KEY", "")`；保留 `gemini_api_key` |
| 8 | `src/ai/advisor.py` | `_resolve_api_key("gemini", secrets)` **移除 fallback 讀 `google_api_key`**，只讀 `secrets["gemini_api_key"]` |
| 9 | `tests/test_services/test_config_svc.py` | 4 個新 validator 各 ~3 case（ok / invalid / unreachable）；DeepSeek 額外 1 case（402 → no_quota）；FinMind wrapper ~3 case；共 ~16 條；**+ 1 條 regression**：`.env` 只有 `GOOGLE_API_KEY` 時 `get_secrets_status()` 內 gemini=false（不再 fallback） |
| 10 | `tests/test_api/test_config_api.py` | `secrets/validate` 新 contract case ~7 條（FinMind 必填空白回 200 + finmind invalid_key 且 saved=[] / FinMind 失敗時其他 provider 不寫入 / 全 ok / 部分成功 / 全失敗 / DeepSeek no_quota 寫入 / onboarding 只送 finmind regression） |
| 11 | `tests/test_advisor.py` | 加 1 條 regression：`AIAdvisor(provider="gemini")` 在 `.env` 只有 `GOOGLE_API_KEY` 沒 `GEMINI_API_KEY` 時 `_provider_error` 應為 "Missing API key for provider 'gemini'."（驗證 fallback 已移除） |
| 12 | `web/src/tests/components/dashboard/token-setup-dialog.test.tsx` | 既有 16 case 配合新 response shape 更新；新增 1 條測「token-setup-dialog 仍用 onboarding 流程不送其他 provider」 |
| 13 | `web/src/tests/components/settings/secrets-section.test.tsx` | 加 1 條 regression：render 後**找不到** `label="Google API Key"` 欄位（驗證移除完成） |
...
| 2 | `api/routers/ai.py` | `/api/ai/status` 改為依 `get_config().ai.enabled` 與 `get_secrets_status()` 中對應 provider 是否 configured 動態回傳；`/api/ai/chat` 改 async，回 `EventSourceResponse(generate())`；`generate` 內 `async for chunk in advisor.stream_chat(request.messages)` → yield `{"event": ..., "data": json.dumps(...)}`；錯誤 try/except 包成 `error` event 而非 HTTPException；`ChatRequest.messages` 改用 `list[ChatMessage]`（Pydantic 嚴格 schema）；advisor 以 FastAPI `Depends(get_advisor)` 注入（見下） |
| 3 | `api/deps.py` | **新增 `get_advisor()` dependency**：`def get_advisor() -> AIAdvisor: return AIAdvisor()`。既有檔案只有 `get_manager()`；此 dependency 是 15-B/15-D 測試 mock advisor 的注入 seam，與其他 router 一致使用 `Depends(...)` pattern |
| 4 | `pyproject.toml` | **新增 `sse-starlette>=2.0`**；確認 `httpx`（async streaming 用）已在 |
...
| 2 | `web/src/hooks/use-ai-chat.ts` | 新增。`useAIChat()` 回 `{ messages, send, abort, streaming }`（**無 top-level `error`**；錯誤透過 `event: error` 寫入最後一則 assistant message 的 `error` 欄位以 inline error bubble 顯示）；`send(text)` 內 `fetch('/api/ai/chat', { method: 'POST', body: JSON.stringify({messages}), signal: ctrl.signal })` → reader.read() 迴圈解 SSE `data:` 行 → 區分 `token` / `done` / `error` event；SSE parser 必須 normalize `\r\n` → `\n` 並支援多行 `data:`（MDN spec） |
