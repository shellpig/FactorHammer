...
echo.
echo Waiting for frontend (3000) and backend (8000) to be ready (max 60s) ...
powershell -NoProfile -Command "$ok=$false; for($i=0;$i -lt 60;$i++){ try { $f=Invoke-WebRequest -Uri 'http://localhost:3000' -UseBasicParsing -TimeoutSec 1 -ErrorAction Stop; $b=Invoke-WebRequest -Uri 'http://localhost:8000/api/health' -UseBasicParsing -TimeoutSec 1 -ErrorAction Stop; if($f.StatusCode -eq 200 -and $b.StatusCode -eq 200){$ok=$true; break} } catch { Start-Sleep -Milliseconds 800 } }; if($ok){ Write-Host '  -> Frontend and backend ready' -ForegroundColor Green } else { Write-Host '  -> Timeout, opening anyway' -ForegroundColor Yellow }"
