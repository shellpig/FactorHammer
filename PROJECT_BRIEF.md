...
| 16-C | ⏳ 待實作 | 整合驗證 + 文件收束：手動驗收 16-B-M1 ~ M7 + 規格書 / 設計方針 / 測試指南 / `PROJECT_BRIEF` 收束（走 verifier 角色）。目前僅 `PROJECT_BRIEF` Phase 進度同步至 16-B；其餘三份文件與整合回歸尚未收 |

## 當前待辦

見 `驗證後已知問題.md`（每次必讀）。

主線：**Phase 1–14-B + 15-A-1 + 15-A-2 + 15-B + 15-C + 15-D + 15-E 全部完成並通過驗證；Phase 16 16-A / 16-B 已完成並通過驗證，16-C（整合回歸 / 文件收尾）尚未實作。** 10-F-2（AI 問答接 LLM）已由 Phase 15-B / 15-C / 15-D 接手完成；AI 問答支援 SSE streaming、停止串流、tool use、tool chip、日線資料自動補抓 / 更新與含息總報酬試算。專案已完全遷移至 Next.js + FastAPI；Streamlit 程式碼與套件已從 codebase 移除。
