...
- Phase 15 15-A-1 / 15-A-2 / 15-B / 15-C 已完成並通過驗證：DeepSeek provider、per-provider secrets validate、AI chat 後端 SSE streaming、前端接 SSE + 停止串流 + 思考中 placeholder 已上線；15-D tool use 尚待實作。
...
| 15-D | ⏳ 待實作 | Chat 啟用 tool use：`stream_chat` 多輪 tool 迴圈（最多 6 輪）+ 4 adapter tool_call delta accumulator；SSE 加 `tool_call` / `tool_result` event；前端可顯示 tool chip；`use-ai-chat.ts` 必須以 `toApiMessages()` helper 過濾 UI-only entries（tool_call / tool_result / error-only / greeting）後才送入 `/api/ai/chat`；**手動驗收只在 DeepSeek 上跑**，其他 3 個 provider 列入「驗證後已知問題.md」未實機驗證 |

## 當前待辦

見 `驗證後已知問題.md`（每次必讀）。

主線：**Phase 1–14-B + 15-A-1 + 15-A-2 + 15-B + 15-C 全部完成並通過驗證。** 10-F-2（AI 問答接 LLM）由 Phase 15-B / 15-C / 15-D 接手；15-B 後端 SSE 與 15-C 前端 SSE 已完成，15-D tool use 尚待實作。專案已完全遷移至 Next.js + FastAPI；Streamlit 程式碼與套件已從 codebase 移除。
...
| 15-D：Chat 啟用 tool use（手動驗收只跑 DeepSeek） | 5249-5328 | `stream_chat` 多輪 tool 迴圈（最多 6 輪）、4 adapter tool_call delta accumulator、SSE `tool_call` / `tool_result` event、前端 tool chip、`use-ai-chat.ts` 必須以 `toApiMessages()` helper 過濾 UI-only entries（tool_call / tool_result / error-only / greeting）後才送入 `/api/ai/chat`、手動驗收只在 DeepSeek 上跑（其他 3 個 provider 由 implementer 完成後 verifier 補檔到「驗證後已知問題.md」）時必讀 |
...
| 15-D：Chat 啟用 tool use | 11348-11541 | `stream_chat` 多輪 tool 迴圈程式碼（最多 6 輪）、4 adapter tool_call delta accumulator、SSE `tool_call` / `tool_result` event 結構、前端 tool chip 樣式、`toApiMessages()` helper 過濾 UI-only entries（tool_call / tool_result / error-only / greeting）的位置與規則時必讀 |
...
| 15-D 測試（Chat 啟用 tool use） | 4667-4757 | 驗 15-D：`stream_chat` 多輪 tool 迴圈 tests（最多 6 輪、超出 raise）+ 4 adapter tool_call delta accumulator tests + SSE `tool_call` / `tool_result` event 結構 tests + `toApiMessages()` helper 過濾 UI-only entries（tool_call / tool_result / error-only / greeting）tests + DeepSeek 手動驗收（其他 3 個 provider 由 implementer 完成後 verifier 補檔至「驗證後已知問題.md」） |
