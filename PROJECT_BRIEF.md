...
| 15-C | ⏳ 待實作 | AI 問答前端接 SSE：新增 `use-ai-chat.ts`（fetch + ReadableStream 解 SSE、`send` / `abort` / `messages` / `streaming`）、移除 `use-mock-chat.ts`；chat-page-client 加取消按鈕、markdown 部分閉合直接交給 `react-markdown` 即時 render；移除 sidebar「後續開放」徽章；訊息歷史不持久化；三處版本同步 `0.7.0` |
...
主線：**Phase 1–14-B + 15-A-1 + 15-A-2 + 15-B 全部完成並通過驗證。** 10-F-2（AI 問答接 LLM）由 Phase 15-B / 15-C / 15-D 接手；15-B 後端 SSE 已完成、前端目前仍走 mock；15-C / 15-D 為嚴格串行。專案已完全遷移至 Next.js + FastAPI；Streamlit 程式碼與套件已從 codebase 移除。
...
2026-05-24 狀態（Phase 15-A-2 + 15-B 完成）：
...
- **15-B 前端尚未切換**：規格將前端接 SSE 拆到 15-C；目前 AI 問答頁仍走 `use-mock-chat.ts`，僅顯示「AI 串接尚未開放（這是 UI 預覽）」mock 文案。15-B 驗收以 curl `--no-buffer` 與 pytest 為準。
